...
mod tests {
    ...
    fn test_fit_simple_case() {
        ...
        model.fit_sgd(&x_data, &targets, 0.1, 100);
    ...
    fn test_fit_multivariate() {
        ...
        // y = 1 + 2*x1 + 3*x2
        let targets = Vector::new(vec![6.0, 8.0, 13.0, 15.0, 20.0, 6.0, 8.0, 13.0, 15.0, 20.0]);
        
        model.fit_sgd(&x_data, &targets, 0.07, 100);
    ...
    let targets = Vector::new(
        ...
        let mut model = LinearRegression::new();
        model.fit_sgd(&inputs, &targets, 0.004, 500);
